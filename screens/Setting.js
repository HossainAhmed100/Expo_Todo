      ...
      <View style={{ paddingVertical: 8, paddingHorizontal: 15 }}>
        <TouchableOpacity onPress={() => navigation.navigate('MyProfile')} style={{backgroundColor: "white",borderRadius: 10,padding: 10,marginBottom: 10,}}>
          <View style={{flexDirection: "row",alignItems: "center",gap: 10,justifyContent: "center",}}>
            {user?.userProfilePic ? <Image source={{uri: user?.userProfilePic}} style={{resizeMode: "cover",width: 50,height: 50,borderRadius: 50,}}/> : 
            <Image source={{uri: "https://i.ibb.co/qdHZgxr/polygon-matic-card-image.jpg"}} style={{resizeMode: "cover",width: 50,height: 50,borderRadius: 50,}}/>
            }
            <View style={{flex: 1,flexDirection: "column",alignItems: "flex-start",gap: 1,}}>
              <Text style={{ fontWeight: 500, fontSize: 16 }} numberOfLines={2}>{user?.userName ? user?.userName : "Hossain Ahmed"}</Text>
              <Text style={{ fontSize: 12, color: "#6B7280" }}>{user?.userPhone ? user?.userPhone : "01850000000"}</Text>
            </View>
